...
export const ATTACHMENT_ACCEPT = '.txt,.md,.csv,.tsv,.json,.log,.docx,.xlsx,.xlsm,.pptx,.pdf,.hwpx,.png,.jpg,.jpeg,.webp,.gif,.mp4,.webm'
...
export function attachmentError(name: string, size: number): string | null {
  ...
  if (!extension || !ATTACHMENT_ACCEPT.split(',').includes(extension)) return 'Attach text, PDF, Office documents, images, or MP4 and WebM videos.'
