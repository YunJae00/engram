...
export function DeveloperHistory({ repo, provider, onClose, onImported }: { repo: DevRepo; provider: DevProvider; onClose(): void; onImported(task: DevSession): void }) {
  ...
  const continueSession = (fork: boolean) => {
    ...
    void api.devCreate({ repoId: repo.id, provider: engine, accountProfile: profile, model: '', mode: 'review', isolate: false, resume: preview.session.id, fork, resumeConfirmed, allFolders }).then(onImported).catch(error => setError(error.message)).finally(() => setBusy(false))
  }
  useEffect(() => { dialog.current?.showModal() }, [])
  useEffect(() => {
    const at = ++revision.current
    setLoading(true); setRows([]); setPreview(null); setError('')
    void api.devExternal(repo.id, engine, allFolders, profile).then(rows => { if (at === revision.current) setRows(rows) }).catch(error => { if (at === revision.current) setError(error.message) }).finally(() => { if (at === revision.current) setLoading(false) })
    return () => { revision.current++ }
  }, [repo.id, engine, allFolders, profile])
