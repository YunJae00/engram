...
import { ArrowUp, Check, ChevronRight, CircleAlert, FilePenLine, Folder, GitBranch, ListChecks, LoaderCircle, Search, Square, Terminal, Wrench, X } from 'lucide-react'
...
export function DeveloperTaskPane({ id, slot, repo, state, active, split, onFocus, onCreated, onClose, onChoose }: {
  id?: string; slot: number; repo?: DevRepo; state: DevState; active: boolean; split: boolean; onFocus(): void; onCreated(task: DevSession): void; onClose(): void; onChoose(id: string | undefined, repo: string): void
}) {
  ...
  return <section className={`dev-pane${active ? ' active' : ''}${!task && !loading ? ' dev-pane-welcome' : ''}`} aria-label={`Development pane ${slot + 1}`} onFocusCapture={onFocus} onPointerDown={onFocus}>
    <header className="dev-header"><div className="dev-heading"><h2 title={task?.title}>{split ? <DeveloperPanePicker title={task?.title ?? (loading ? 'Loading session…' : 'New session')} current={id} state={state} onChoose={onChoose} /> : task?.title ?? (loading ? 'Loading session…' : 'New session')}</h2><small className="dev-folder-context" title={task?.cwd ?? repo?.path}><Folder size={12} aria-hidden /><span>{task?.cwd ?? repo?.path ?? 'Choose a folder'}</span>{chosenAccess.isolate && <span className="dev-folder-badge" title={task?.branch}>Worktree</span>}</small></div>
      <div className="dev-actions">{task && <><DeveloperPopover label="Branch session" trigger={<GitBranch size={16} />} disabled={busy || running || !task.runtimeId}>{close => <><strong>Branch conversation</strong><button className="dev-menu-row" onClick={() => { close(); void action(async () => onCreated(await api.devFork(task.id, false))) }}><Folder size={16} /><span>Same folder<small>New conversation, shared files. Works without Git.</small></span></button><button className="dev-menu-row" onClick={() => { close(); void action(async () => onCreated(await api.devFork(task.id, true))) }}><GitBranch size={16} /><span>Separate worktree<small>Isolated files from the current Git commit.</small></span></button></>}</DeveloperPopover><button className="dev-control" disabled={busy} onClick={() => void action(async () => { setGit(await api.devGit(task.id)); setSelected([]) })}>Changes</button></>}{split && <button className="dev-control" aria-label="Close pane" onClick={onClose}><X size={16} /></button>}</div>
    </header>
    {error && <div className="dev-error" role="alert">{apiErrorText(error)}<button className="dev-control" aria-label="Dismiss error" onClick={() => setError('')}><X size={14} /></button></div>}
    <div className="dev-log" ref={log} onScroll={event => { const view = event.currentTarget; follow.current = view.scrollHeight - view.scrollTop - view.clientHeight < 100 }}>
      ...
      {busy && !running && <p className="dev-working" role="status"><LoaderCircle size={14} className="spin" aria-hidden />Working…</p>}
    </div>
