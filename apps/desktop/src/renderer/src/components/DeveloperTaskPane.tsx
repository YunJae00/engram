...
import { ArrowUp, ChevronDown, Folder, GitBranch, LoaderCircle, Settings, Sparkles, Square, X } from 'lucide-react'
  ...
  return <section className={`dev-pane${active ? ' active' : ''}`} aria-label={`Development pane ${slot + 1}`} onFocusCapture={onFocus} onPointerDown={onFocus}>
    ...
    <div className="dev-composer">
      ...
      <div className="dev-composer-toolbar"><div className="dev-composer-tools">
        <DeveloperPopover label="Skills" disabled={!task || running || busy} trigger={<Sparkles size={16} />}>{close => task && <Skills id={task.id} onSelect={value => { write(value); close(); input.current?.focus() }} />}</DeveloperPopover>
