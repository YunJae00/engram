...
export function DeveloperTaskPane({ id, slot, repo, state, active, split, onFocus, onCreated, onClose, onChoose }: {
  ...
  return <section className={`dev-pane${active ? ' active' : ''}${!task && !loading ? ' dev-pane-welcome' : ''}`} aria-label={`Development pane ${slot + 1}`} onFocusCapture={onFocus} onPointerDown={onFocus}>
    <header className="dev-header"><div className="dev-heading"><h2 title={task?.title}>{split ? <DeveloperPanePicker title={task?.title ?? (loading ? 'Loading session…' : 'New session')} current={id} state={state} onChoose={onChoose} /> : task?.title ?? (loading ? 'Loading session…' : 'New session')}</h2><small title={task?.cwd ?? repo?.path}><Folder size={12} />{repo?.name ?? 'Choose a folder'}{task?.branch && <span title={task.branch}> · Worktree</span>}</small></div>
