...
export function AppSidebar({ open, onToggle, onOpenPalette, onOpenSettings, onOpenRoutines, selectedRoutineId, onSelectRoutine }: Props) {
  ...
  return <aside className={`app-sidebar${open ? ' open' : ''}`} data-testid="app-sidebar" aria-hidden={!open}>
    ...
    <div className="app-sidebar-head"><WorkspaceSwitcher activity={activity} onNavigate={navigate} onOpenRoutines={onOpenRoutines} onOpenPalette={onOpenPalette} /><WorkspaceMode developer={developer} onChange={setActivity} /><button className="sidebar-icon-button" data-testid="app-sidebar-close" title={t('rail.hide')} aria-label={t('rail.hide')} onClick={onToggle}><PanelLeftClose size={17} strokeWidth={1.8} aria-hidden /></button></div>
