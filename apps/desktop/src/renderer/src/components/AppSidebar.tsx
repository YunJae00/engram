...
export function AppSidebar({ open, onToggle, onOpenPalette, onOpenSettings, onOpenRoutines, selectedRoutineId, onSelectRoutine }: Props) {
  ...
  const collection = (kind: SidebarKind) => <SidebarCollection key={kind} kind={kind} newFolder={creating[kind]}
    items={kind === 'chat' ? matchingBots.map(bot => ({ id: bot.id, name: bot.name, active: bot.id === selectedId && (activity === 'bots' || activity === 'mission'), content: <SidebarConversation bot={bot} state={activityOf(bot)} /> })) : matchingRoutines.map(routine => ({ ...routine, active: routine.id === selectedRoutineId, content: <><span className="sidebar-routine-icon"><Repeat2 size={17} strokeWidth={1.6} aria-hidden /></span><span className="sidebar-routine-copy"><strong>{routine.name}</strong><small>{routine.steps.length} steps{routine.lastOutcome && ` · ${routine.lastOutcome === 'done' ? 'Last run completed' : routine.lastOutcome === 'aborted' ? 'Paused' : 'Needs attention'}`}</small></span></> }))}
