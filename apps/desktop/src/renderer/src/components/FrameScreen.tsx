...
export function FrameScreen({ className, lane, source }: { className?: string; lane?: string; source?: FrameSource }) {
  ...
  useEffect(() => {
    ...
    const paint = (source: HTMLImageElement, width: number, height: number): void => {
    ...
    const read = (data: string): void => {
      ...
      // Let the image decoder consume base64 directly: copying every byte
      // in JavaScript blocks input while several large previews arrive.
