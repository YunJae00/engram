...
it('keeps the task alive when an optional skills lookup times out', async () => {
  vi.useFakeTimers()
  const test = runtime(), ended = vi.fn(), notify = vi.fn()
  const rpc = new DevRpc('runtime', { cwd: '.', env: {} }, notify, async () => ({ decision: 'decline' }), ended)
  const skills = rpc.send('skills/list', {}, 100).catch(error => error.message)
  await vi.advanceTimersByTimeAsync(101)
  expect(await skills).toContain('did not respond')
  expect(test.child.kill).not.toHaveBeenCalled()
  test.reply({ method: 'item/agentMessage/delta', params: { delta: 'Still working' } })
  expect(notify).toHaveBeenCalledWith('item/agentMessage/delta', { delta: 'Still working' })
  const next = rpc.send('turn/start', {})
  test.reply({ id: 1, result: { late: true } })
  test.reply({ id: 2, result: { turn: { id: 'next' } } })
  expect(await next).toEqual({ turn: { id: 'next' } })
