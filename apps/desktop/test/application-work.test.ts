...
it('uses the actual app window without acquiring input, follows geometry and stops the same lane', async () => {
  ...
  expect(fake.request.mock.calls.every(([method]) => ['inputState', 'inspectWindow', 'applicationFrame'].includes(method))).toBe(true)
  expect(fake.request).toHaveBeenCalledWith('applicationFrame', { window: '100', pid: 23 })
  expect(fake.status.application?.nativeFrame).toBe(true)
...
it('keeps document work independent when the person types in another app', async () => {
  fake.request.mockResolvedValueOnce(target).mockResolvedValueOnce({ window: '101' }).mockResolvedValueOnce({ escaped: false }).mockResolvedValueOnce({ ...target, foreground: false })
  const work = applicationWork('one')
  await work.show('100', 'Excel')
  fake.request.mockImplementation(async (method: string) => method === 'inputState' ? { escaped: false, intervention: 'new user input' } : { ...target, foreground: false })
  await vi.advanceTimersByTimeAsync(101)
  expect(fake.status.application?.visible).toBe(true)
  expect(work.signal.aborted).toBe(false)
  await work.show('100', 'Excel')
  expect(fake.request.mock.calls.some(([method]) => method === 'activateWindow')).toBe(false)
  fake.request.mockImplementation(async (method: string) => method === 'inputState' ? { escaped: false } : { ...target, minimized: true })
  await vi.advanceTimersByTimeAsync(101)
  expect(fake.status.state).toBe('running')
  expect(fake.status.application?.visible).toBe(false)
})
