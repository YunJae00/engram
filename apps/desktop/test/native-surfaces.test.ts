...
it('keeps idle layout work at heartbeat rate and coalesces resize events', async () => {
  ...
  vi.stubGlobal('document', { body: {}, querySelectorAll: () => [], elementFromPoint: () => element, addEventListener: vi.fn(), removeEventListener: vi.fn() })
  vi.stubGlobal('MutationObserver', class { constructor(callback: () => void) { mutation = callback } observe() {} disconnect() {} })
  ...
  const unmount = mountNativeSurface(element as unknown as HTMLElement, 'fixture')
  try {
