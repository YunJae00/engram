...
it('keeps idle layout work at heartbeat rate and coalesces resize events', async () => {
  ...
  vi.stubGlobal('document', { body: {}, querySelectorAll: () => [], elementFromPoint: () => element, addEventListener: (name: string, callback: (event: Event) => void) => listeners.set(name, callback), removeEventListener: vi.fn() })
