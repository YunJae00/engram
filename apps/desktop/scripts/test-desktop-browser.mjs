...
export async function testDesktopBrowser(helper, desktop, output, until) {
  ...
  try {
    ...
    let view = await helper.request('observe', target)
    assert.ok(view.nodes.some(node => node.name === 'Browser entry'), 'Owned browser input must expose accessibility')
    ...
    view = await helper.request('observe', bound)
    assert.ok(view.nodes.some(node => node.value === 'Browser 한글 input'), 'Native typing must reach the browser field')
    ...
    view = await helper.request('observe', bound)
    assert.ok(view.nodes.some(node => node.name === 'Browser count 1'), 'Native click must update the browser')
