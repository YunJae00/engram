...
test('rejects unsupported inputs visibly and accepts an image', async () => {
  ...
  await expect(page.getByLabel('Attached files').getByRole('img', { name: 'pixel.png' })).toBeVisible()
...
test('splitting a conversation preserves and sends its multiline draft and attachments', async () => {
  ...
  await expect(page.getByTestId('bots-input')).toHaveValue('')
  await expect(page.locator('.bots-view .bubble-msg.user').last().locator('.chat-user-text')).toHaveText(draft)
...
test('long pasted text becomes a readable attachment without filling the composer', async () => {
  ...
  expect(await video.evaluate(async node => { const player = node as HTMLVideoElement; await player.play(); player.pause(); return player.videoWidth })).toBe(100)
