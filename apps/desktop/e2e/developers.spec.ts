...
test('connected accounts are compact and isolated profiles require an explicit restart', async () => {
  await page.getByTestId('activity-settings').click()
  ...
  await page.getByRole('button', { name: 'Manage ChatGPT accounts', exact: true }).click()
  ...
  await expect(page.getByRole('radio', { name: 'Work account' })).toBeChecked()
  await expect(page.getByRole('button', { name: 'Use profile and restart' })).toBeEnabled()
  ...
  await page.getByRole('dialog', { name: 'Account profiles' }).getByRole('button', { name: 'Cancel', exact: true }).click()
  await page.keyboard.press('Escape')
})
