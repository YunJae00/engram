...
test('conversation and filing selections persist without changing each other', async () => {
  ...
  await expect(page.getByTestId('model-picker-menu')).toHaveCount(0)
  await page.locator('.bots-write').getByTestId('effort-picker').click()
  await page.getByTestId('effort-pick-high').click()
  await expect(page.getByTestId('effort-picker-menu')).toHaveCount(0)
  await expect(page.locator('.bots-write').getByTestId('effort-picker')).toHaveText('High')
  await page.reload()
