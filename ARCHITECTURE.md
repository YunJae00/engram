...
`node-llama-cpp` runs **in-process** inside Electron's main process — not as a spawned server. That is deliberate: corporate endpoint protection commonly blocks unsigned downloaded executables, which kills the sidecar pattern but leaves the signed Electron binary alone. Models are downloaded through Chromium's `net.fetch` because TLS-inspecting proxies break Node's fetch.

The engine is loaded lazily, serialized to one call at a time, and unloaded after ten idle minutes.
...
apps/desktop/src/
  main/                    IPC, local model, semantic indexer, capture watchers
