...
describe('reading a transcript span', () => {
  it('keeps the conversation and drops the bookkeeping', async () => {
    const span =
    ...
    const { turns } = await parseSessionSpan(span)
    expect(turns.map((t) => t.role)).toEqual(['user', 'assistant'])
    expect(turns[0]!.text).toBe('왜 이게 안 되지?')
    expect(turns[1]!.text).toBe('토큰이 만료돼서임.')
  })

  it('drops the model thinking to itself and its tool mechanics', async () => {
  ...
  it('skips a turn that carried no prose at all', async () => {
    const { turns } = await parseSessionSpan(assistant([{ type: 'tool_use', name: 'Read', input: {} }]))
    expect(turns).toEqual([])
  })

  // The file is being appended to while we read it.
  it('leaves a half-written final line for the next read', async () => {
    const whole = user('첫 줄')
    const span = whole + '{"type":"user","message":{"role":"user","content":"잘린'
    const { turns, consumed } = await parseSessionSpan(span)
    expect(turns).toHaveLength(1)
    expect(consumed).toBe(Buffer.byteLength(whole, 'utf8'))
  })

  it('reports consumed bytes in BYTES, not characters', async () => {
    const span = user('한글은 세 바이트다')
    const { consumed } = await parseSessionSpan(span)
...
describe('what travels to the engine', () => {
  it('truncates a pasted wall of text so it cannot crowd out the reasoning', async () => {
    const rendered = renderSpan([{ role: 'user', text: 'x'.repeat(5_000), at: '' }])
    expect(rendered.length).toBeLessThan(1_700)
    expect(rendered).toContain('…')
  })

  it('keeps the most RECENT turns when a span is long', async () => {
...
describe('naming the project a transcript belongs to', () => {
  it('recovers the working directory leaf from the flattened folder name', async () => {
