  ...
  routine: z
    .object({
        ...
        task: z.object({
          ...
          checks: z.array(z.string().max(500)).max(8).optional(),
          context: z.array(z.string().max(4000)).max(4).optional(),
          execution: z.object({ engine: z.enum(['claude', 'codex']), model: z.string().max(200), effort: z.enum(['none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', 'ultra']).optional() }).optional(),
        }).optional(),
