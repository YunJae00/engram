  ...
  return {
    ...
    async run(_args, context) {
      ...
      return JSON.stringify({
        ...
        liveDocumentApi: { available: tools.some(tool => tool.name === 'read_live_document'), tools: tools.filter(tool => ['read_live_document', 'edit_live_document'].includes(tool.name)).map(tool => ({ name: tool.name, description: tool.description })), fallback: 'Use available desktop tools for unsupported applications and operations. Never edit the on-disk file to modify an open unsaved document.' },
