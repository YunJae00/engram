...
export const J8_INSTRUCTION = [
  ...
  "",
  "Each item in 'loops' arrives with part of its note body as `excerpt`. Draw the line's content FROM that excerpt — a ticket number, a PR number, the name of an item still undone, the concrete next action. Use only the next action recorded in that excerpt, not a fixed workflow.",
  "When one piece of work is scattered across several notes, fold it into ONE line only when the notes explicitly refer to the same work. Matching ticket numbers alone do not establish identity across different project prefixes. Do not mechanically emit a line per note. The user needs a list of things to do, not a list of notes.",
