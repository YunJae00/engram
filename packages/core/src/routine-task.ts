...
export function routineTask(goal: string, steps: TurnStep[], context: string[] = [], requests: string[] = []): NonNullable<Routine['task']> {
  ...
  const checks = [...new Set(successful.flatMap(step => step.tool === 'task_plan' && Array.isArray(step.args['phases']) ? step.args['phases'].filter((value): value is string => typeof value === 'string').map(value => withoutSecrets(value, source).slice(0, 500)) : []))].slice(0, 8)
